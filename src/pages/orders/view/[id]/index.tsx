...
import { deleteVoiceNoteById } from 'apiSdk/voice-notes';
...
function OrderViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<OrderInterface>(
    ...
    () =>
      getOrderById(id, {
        relations: ['user', 'organization', 'voice_note'],
      }),
  );

  const voice_noteHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('voice_note', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/voice-notes/create?order_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.voice_note?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/voice-notes/view/${record.id}`)}
                            key={record.id}
                          >
                            ...
                            <Td>
                              {hasAccess('voice_note', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/voice-notes/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('voice_note', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    voice_noteHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
